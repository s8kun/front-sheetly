...
interface ActivityItemProps {
    item: Sheet & { isLazy?: boolean; chapterNumber?: number; isFolder?: boolean };
    type: 'file' | 'exam';
}
...
const ActivityItem = memo(({item, type}: ActivityItemProps) => {
    ...
    const handleDelete = async (e: React.MouseEvent) => {
        ...
        if (item.isLazy && item.chapterNumber) {
            ...
            if (!confirm(`هل أنت متأكد من حذف الشابتر ${item.chapterNumber} بجميع ملفاته؟`)) return;
    ...
    return (
        <motion.div
            ...
            className={`group relative border p-4 rounded-[2rem] shadow-sm transition-all cursor-pointer ${
                isDeleting ? 'opacity-50 grayscale' : 'bg-white/70 backdrop-blur-sm border-white/60 hover:shadow-md hover:border-indigo-200'
            }`}
        >
            {isAdmin && (
                <button 
            ...
            <div className="flex items-start gap-4">
                <div
                    className={`shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-md ${
                        type === 'file' ? 'bg-indigo-500' : 'bg-rose-500'
                    }`}>
                    {isFolder ? (
            ...
            <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                ...
                <button
                    ...
                    className={`text-xs px-4 py-2 rounded-xl font-black transition-all ${
                        isDownloading || isDeleting ? 'bg-slate-200 text-slate-400' : 'bg-slate-100 hover:bg-indigo-600 hover:text-white text-slate-600'
                    }`}
                >
