...
import { 
    ...
    ChevronRight, ChevronLeft, Shield, User as UserIcon,
    X, Check, AlertCircle, Home
} from 'lucide-react';
...
export default function UsersManagement() {
    ...
    const fetchUsers = async (token: string) => {
        setIsLoading(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/users?page=${currentPage}`, {
    ...
    return (
        <div dir="rtl" className={`${cairo.className} min-h-screen bg-slate-50 p-6 md:p-10`}>
            {/* Breadcrumb العلوية */}
            <nav className="mb-8 flex justify-end" aria-label="Breadcrumb">
                <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse bg-white/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/50 shadow-sm">
                    ...
                    <li>
                        <div className="flex items-center">
                            ...
                            <Link href="/admin/dashboard" className="inline-flex items-center text-xs font-bold text-slate-500 hover:text-indigo-600 transition-colors">
                                لوحة التحكم
                            </Link>
                        </div>
                    </li>
            ...
            <header className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    ...
                    <p className="text-slate-500">عرض وإضافة وتعديل صلاحيات المستخدمين</p>
                </div>
                <button 
                    onClick={() => handleOpenModal('create')}
                    className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                >
            ...
            <main className="max-w-6xl mx-auto">
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <table className="w-full text-right border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 border-b border-slate-100">
                                    ...
                                    <th className="p-6 font-bold text-slate-600">الرتبة</th>
                                    <th className="p-6 font-bold text-slate-600">تاريخ الانضمام</th>
                                    <th className="p-6 font-bold text-slate-600 text-center">الإجراءات</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    {isLoading ? (
                                        [1, 2, 3, 4, 5].map(i => (
                                            <tr key={i} className="animate-pulse">
                                                <td colSpan={4} className="p-4"><div className="h-12 bg-slate-50 rounded-xl" /></td>
                                            </tr>
                                        ))
                                    ) : users.length > 0 ? (
                                        users.map((user) => (
                                            ...
                                            >
                                                ...
                                                <td className="p-6">
                                                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${user.role === 'admin' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-600'}`}>
                    ...
                    {pagination && pagination.last_page > 1 && (
                        <div className="p-6 bg-slate-50/50 flex justify-center items-center gap-4">
                            <button 
                            ...
                            <span className="text-sm font-bold text-slate-600">صفحة {currentPage} من {pagination.last_page}</span>
                            <button 
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        <motion.div 
                            ...
                            className="relative bg-white rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto"
                        >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-bold"
                                    >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">
                                            {modalMode === 'edit' ? 'كلمة المرور الجديدة (اختياري)' : 'كلمة المرور'}
                                        </label>
                                        <input 
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            type="password" required={modalMode === 'create' || formData.password !== ''}
                                ...
                                <div className="flex gap-3 pt-6">
                                    ...
                                    <button 
                                        ...
                                        className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                    >
                                        {isSubmitting ? 'جاري الحفظ...' : (modalMode === 'create' ? 'إنشاء المستخدم' : 'حفظ التعديلات')}
