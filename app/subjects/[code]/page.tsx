...
export default function SubjectPage({params}: { params: Promise<{ code: string }> }) {
    const resolvedParams = use(params);
    const subjectCode = resolvedParams.code;
    const searchParams = useSearchParams();
    const router = useRouter();

    const activeTab = (searchParams.get('tab') as 'chapters' | 'midterms' | 'finals') || 'chapters';
    ...
    const handleDeleteSubject = async () => {
        if (!confirm('هل أنت متأكد من حذف هذه المادة نهائياً؟ سيتم حذف جميع ملفاتها أيضاً!')) return;
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (result.chapters && result.chapters.length > 0) {
                        const chaptersPromises = result.chapters.map(async (item: any, index: number) => {
            ...
            <main className="relative z-10">
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <Link href="/subjects" className="text-indigo-600 hover:underline text-sm">العودة لقائمة المواد</Link>
                    </div>
                ) : data ? (
                    <>
                        <div className="text-center mb-10 relative">
                            ...
                            <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-sm font-bold tracking-wider uppercase">{data.subject.code}</span>
                            <h1 className="text-3xl md:text-4xl font-black text-slate-800 mt-3 mb-2">{data.subject.name}</h1>
                        </div>
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`relative px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
                                            activeTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-indigo-600'
                                        }`}
                                    >
                                        {activeTab === tab.id && (
                                            <motion.div
                        ...
                        <div className={activeTab === 'chapters' ? 'block' : 'hidden'}>
                            ...
                            <Activity items={enrichedChapters} type="file" emptyMessage="لا توجد شيتات مضافة بعد"/>
                        </div>
                        <div className={activeTab === 'midterms' ? 'block' : 'hidden'}>
                            <Activity items={midtermsData} type="exam" emptyMessage="لا توجد امتحانات جزئية"/>
                        </div>
                        <div className={activeTab === 'finals' ? 'block' : 'hidden'}>
                            <Activity items={finalsData} type="exam" emptyMessage="لا توجد امتحانات نهائية"/>
                        </div>
                    </>
                ) : null}
            </main>
